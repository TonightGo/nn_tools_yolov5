...
class TransLog(object):
    ...
    def add_blobs(self, blobs, name='blob', with_num=True):
        ...
        for blob in blobs:
            ...
            blob_id = int(id(blob))
            if with_num:
                rst.append('{}{}'.format(name, len(self._blobs)))   # blob 名称
            else:
        ...
        for stack in traceback.walk_stack(None):
            if 'self' in stack[0].f_locals:
                ...
                if layer in layer_names:
                    ...
                    save_output = True
                    save_layer_name = layer_names[layer]
                    print("Processing Layer: " + layer_names[layer])
                    break
        out = self.obj(self.raw, *args, **kwargs)
        if save_output:
            layer_outputs[save_layer_name] = out
...
for op_name in F.__dict__:
    if op_name in F_supported:
        raw_func=getattr(F, op_name)    # pytorch的原函数地址
        transfer_func=globals()['_'+ op_name]    # 包装函数地址
        op_wrapper=Rp(raw_func,transfer_func)
        setattr(F, op_name, op_wrapper) # 将新的包装函数地址替换到原函数
    else:
...
def torch_argmax(raw, input, dim=0):
    x = raw(input, dim)
    if not NET_INITTED:
        return x
    layer_name = log.add_layer(name='argmax')
    top_blobs = log.add_blobs([x], name='argmax_blob')
    layer = caffe_net.Layer_param(name=layer_name, type='ArgMax',
                                  bottom=[log.get_blobs(input)], top=top_blobs)
    layer.param.argmax_param.axis = dim
    log.cnet.add_layer(layer)
    return x

def torch_sigmoid(raw, input):
    x = raw(input)
    if not NET_INITTED:
        return x
...
def torch_squeeze(raw,*args):
    pass

def torch_permute(raw, input, *args):
    x = raw(input, *args)
    if not NET_INITTED:
        return x
    ...
    for order in list(args):
        layer.param.permute_param.order.append(order)
    log.cnet.add_layer(layer)
    return x

def torch_arange(raw, input, *args):
    pass
...
def _permute(input, *args):
    x = raw_tensor_magic_op['permute'](input, *args)
    if not NET_INITTED:
        return x
    layer_name=log.add_layer(name='permute')
    top_blobs=log.add_blobs([x],name='permute_blob')
...
def _sigmoid(input):
    x = raw_tensor_magic_op['sigmoid'](input)
    if not NET_INITTED:
        return x
    layer_name = log.add_layer(name='sigmoid')
    top_blobs = log.add_blobs([x], name='sigmoid_blob')
...
def _lt(input,*args):
    x = ___lt__(input, *args).float()
    return ___mul__(input, x)
...
def ___lt__(input, *args):
    x = raw_tensor_magic_op['__lt__'](input, *args)
    if not NET_INITTED:
        return x
    layer_name = log.add_layer(name='threshold')
    top_blobs = log.add_blobs([x], name='threshold_blob')
...
# nn operate, example: Sequential
nn_supported = [
    'Sequential',
]

# def Sequential_forward(raw, input):
#     # 返回最后一个blobs
#     x = raw(input)
#     # print(raw.__code__.co_varnames[0])
#     pytorch_name = layer_names[0]
#     return layer_outputs[pytorch_name]

# for op_name in nn.__dict__:
#     if op_name in nn_supported:
#         raw_func = getattr(nn, op_name).forward   # pytorch的原函数地址
#         transfer_func = globals()[op_name + '_forward']
#         op_wrapper = Rp(raw_func, transfer_func)
#         setattr(getattr(nn, op_name), raw_func.__name__, op_wrapper) # 将新的包装函数地址替换到原函数

def Sequential_forward(self, inputs):
    # 返回最后一个blobs
    for module in self:
        inputs = module(inputs)
        pytorch_name = layer_names[module]
    return layer_outputs[pytorch_name]
...
#         raw_func = getattr(nn, op_name).forward   # pytorch的原函数地址
#         transfer_func = globals()[op_name + '_forward']
#         getattr(nn, op_name).forward = MethodType(Sequential_forward, getattr(nn, op_name)) # 指向类型对象，是op_name类的函数绑定，self指向该type对象，而不是指向该type实例对象。相当于绑定了一个静态函数。
#         # op_wrapper = Rp(raw_func, transfer_func)
#         # setattr(getattr(nn, op_name), raw_func.__name__, op_wrapper) # 将新的包装函数地址替换到原函数
...
def Bottleneck_forward(self, inputs):
    save_output = False
    save_layer_name = None
    for stack in traceback.walk_stack(None):
        if 'self' in stack[0].f_locals:
            ...
            if layer in layer_names:
                ...
                print("Processing Layer: " + layer_names[layer])
                break
    out = inputs + self.cv2(self.cv1(inputs)) if self.add else self.cv2(self.cv1(inputs))
    if save_output:
        layer_outputs[save_layer_name] = out
    return out
...
def trans_net(net,input_var,name='TransferedPytorchModel'):
    ...
    for name, layer in net.named_modules():
        print(':::::', type(layer).__name__, name)
        if type(layer).__name__ in nn_supported:
            layer.forward = MethodType(globals()[type(layer).__name__ + '_forward'], layer)
            
        if type(layer).__name__ in yolov5_supported:
            layer.forward = MethodType(globals()[type(layer).__name__ + '_forward'], layer)
